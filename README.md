...
`{ schemaVersion, agent, revision, client, storageScope }`. `storageScope` is an
opaque server-derived value used for browser chat and panel persistence.
...
infrastructure endpoint URLs. Provider and compression credentials/endpoints stay
in server-only handler options and environment variables.
...
use `getHostedConfig`, setting `config.runtime.compression` in the published
canonical record turns it on with no redeploy. A value set in code always wins.
